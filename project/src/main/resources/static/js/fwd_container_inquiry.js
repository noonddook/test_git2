document.addEventListener('DOMContentLoaded', () => {
	...
	containerList.addEventListener('click', async (e) => {
	            ...
	            if (detailsData.length === 0) {
	                ...
	                detailsData.forEach(item => {
	                    ...
	                    if (item.external) {
	                        ...
	                        if (isResalePossible) {
	                             manageButtonHtml += `<button class="btn btn-xs btn-outline btn-resale-from-details" data-offer-id="${item.offerId}">마켓에 올리기</button>`;
	                        }
	                        manageButtonHtml += ` <button class="btn btn-xs btn-outline btn-move-cargo" data-offer-id="${item.offerId}" data-item-name="${item.itemName}" data-item-cbm="${item.cbm}">화물 이동</button>`;
	    ...
	    // '화물 이동' 버튼 클릭 이벤트 리스너 (else if로 추가)
		else if (target.matches('.btn-move-cargo')) {
		    const button = target;
		    const card = button.closest('.container-card');
		    const offerId = button.dataset.offerId;
		    const itemName = button.dataset.itemName;
		    const itemCbm = button.dataset.itemCbm;
		    
		    const modalTemplate = document.getElementById('cargo-transfer-modal-template');
		    const modalClone = modalTemplate.content.cloneNode(true);
		    const modalWrapper = modalClone.querySelector('.modal-wrapper');
		    
		    // 모달에 정보 채우기
		    modalWrapper.querySelector('#transfer-item-name').textContent = itemName;
		    modalWrapper.querySelector('#transfer-item-cbm').textContent = `${itemCbm} CBM`;
		    
		    const currentContainerId = card.querySelector('.info-group:first-child .value').textContent;
		    const currentRoute = card.querySelector('.route .value').textContent;
		    
		    // [✅ 핵심 수정] 출발 컨테이너의 ETA 정보 가져오기
		    const fromContainerEtaText = Array.from(card.querySelectorAll('.info-group .label')).find(el => el.textContent === 'ETA:').nextElementSibling.textContent;
		    const fromContainerEta = new Date(fromContainerEtaText.replace(/\.\s/g, '-')); // "yyyy. M. d." -> "yyyy-MM-dd"
		    
		    const select = modalWrapper.querySelector('#target-container-select');
		    
		    document.querySelectorAll('.container-card').forEach(c => {
		        const isScheduled = !c.querySelector('.progress-tracker');
		        const containerId = c.querySelector('.info-group:first-child .value').textContent;
		        const route = c.querySelector('.route .value').textContent;
		        
		        // [✅ 핵심 수정] 이동 가능한 컨테이너인지 필터링 조건 강화
		        if (isScheduled && containerId !== currentContainerId && route === currentRoute) {
		            const toContainerEtaText = Array.from(c.querySelectorAll('.info-group .label')).find(el => el.textContent === 'ETA:').nextElementSibling.textContent;
		            const toContainerEta = new Date(toContainerEtaText.replace(/\.\s/g, '-'));

		            if (toContainerEta <= fromContainerEta) {
		                const option = new Option(`${containerId} (ETA: ${toContainerEtaText})`, containerId);
		    ...
		    // 모달 닫기 이벤트
		    modalWrapper.querySelector('.btn-close').addEventListener('click', () => modalWrapper.remove());
		    modalWrapper.querySelector('.btn-cancel').addEventListener('click', () => modalWrapper.remove());

		    // '이동 확정' 버튼 클릭 이벤트 (기존과 동일)
		    modalWrapper.querySelector('#btn-confirm-transfer').addEventListener('click', async () => {
		        const toContainerId = select.value;
		        if (!toContainerId) {
		            alert('이동할 컨테이너를 선택하세요.');
		            return;
		        }

		        try {
		            const response = await fetch('/api/fwd/cargo-transfer', {
		                method: 'POST',
		                headers: { 'Content-Type': 'application/json' },
		                body: JSON.stringify({ offerId: offerId, toContainerId: toContainerId })
		            });
		            const message = await response.text();
		            alert(message);
		            if(response.ok) {
